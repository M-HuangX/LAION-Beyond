...
def strip_state_dict(state_dict, strip_key=''):
    """
    Strip the prefix 'strip_key' from the keys in the state dictionary.

    :param state_dict: The input state dictionary
    :param strip_key: The prefix to be removed from the keys
    :return: A new state dictionary with updated keys
    """
    new_state_dict = {}
    for k, v in state_dict.items():
        if k.startswith(strip_key):
            new_key = k[len(strip_key):]
        else:
            new_key = k
...
def init_experiment(args, runner_name=None, exp_id=None):
    ...
    if args.dataset_name == 'laion_beyond':
        log_dir = os.path.join(root_dir, args.dataset_name, args.subfield)
    else:
        log_dir = os.path.join(root_dir, args.dataset_name)
