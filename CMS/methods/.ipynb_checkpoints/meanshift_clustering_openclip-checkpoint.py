...
def iterative_meanshift_inductive(model, loader, val_loader, args):
    ...
    with torch.no_grad():
        ...
        # 筛选出需要的高置信度新类数据并保存
        save_filtered_data(test_f_preds, test_f_targets, test_all_uq_idxs, mask_cls, args)


        # print(f'ACC with predicted number of clusters: All {final_acc[1][0]:.4f} | Old {final_acc[1][1]:.4f} | New {final_acc[1][2]:.4f}')


def save_filtered_data(test_f_preds, test_f_targets, test_all_uq_idxs, mask_cls, args):
    ...
    # 将数据保存为 .npz 文件
    np.savez('filtered_clustering_results.npz', 
...
if __name__ == "__main__":
    ...
    args.model_name = "/202331510162/llm/hx/GCD_DATA/log/cms/" + args.dataset_name + "/checkpoints/model_best.pt"
