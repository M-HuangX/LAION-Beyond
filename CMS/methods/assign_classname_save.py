...
def compute_image_embeddings(model, preprocess, image_paths, cluster_ids, device):
    ...
    for cluster_id in tqdm(unique_cluster_ids, desc="Computing image embeddings"):
        ...
        # 如果聚类中的图片数量超过60，随机采样60张
        if len(cluster_paths) > 60:
            cluster_paths = random.sample(cluster_paths, 60)
...
def main(args):
    # 构建NPZ文件路径
    npz_file_path = os.path.join(args.laion_beyond_root, f'{args.subfield}_filtered_clustering_results.npz')
    
    # 加载NPZ文件
    data = load_npz(npz_file_path)
    image_paths = data['i_paths'].tolist()  # 转换为 Python 列表
    cluster_ids = data['test_f_preds']
    ...
    # 构建输出JSON文件路径
    output_json_path = os.path.join(args.laion_beyond_root, f'{args.subfield}_clustering_results.json')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--laion_beyond_root", type=str, default="/202331510162/llm/hx/LAION_Beyond_5B/", 
