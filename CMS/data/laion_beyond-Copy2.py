...
class CustomAnimalsDataset(VisionDataset):
    def __init__(self, root, transform=None, target_transform=None):
        super(CustomAnimalsDataset, self).__init__(root, transform=transform, target_transform=target_transform)
        self.root = root
...
def create_dataset(root, oov_dir, iv_dir):
    dataset = CustomAnimalsDataset(root)
...
def get_animals_datasets(train_transform, test_transform, train_classes=(0, 1, 8, 9),
                       prop_train_labels=0.8, split_train_val=False, seed=0):
    root = laion_beyond_root
    split_file = os.path.join(root, f'animals_split_seed_{seed}.json')
    ...
    if os.path.exists(split_file):
        ...
        oov_dir, iv_dir = find_dataset_info(root, "Animals")
...
def get_animals_datasets_with_gcdval(train_transform, test_transform, train_classes=range(80), 
                                   prop_train_labels=0.8, split_train_val=True, seed=0, val_split=0.1):
    root = laion_beyond_root
    split_file = os.path.join(root, f'animals_split_gcdval_seed_{seed}.json')
...
if __name__ == '__main__':
    ...
    datasets = get_animals_datasets(root, train_transform, test_transform, split_train_val=True, seed=42)
    
    for name, dataset in datasets.items():
        if dataset is not None:
            print(f"{name}: {len(dataset)}")
    
    datasets_gcdval = get_animals_datasets_with_gcdval(root, train_transform, test_transform, seed=42)
