...
from data.cifar import get_cifar_10_datasets, get_cifar_100_datasets, get_cifar_10_datasets_with_gcdval, get_cifar_100_datasets_with_gcdval
...
from data.laion_beyond import get_laion_beyond_datasets, get_laion_beyond_datasets_with_gcdval, find_dataset_info, load_merged_mapping
...
from config import osr_split_dir, laion_beyond_root
...
get_dataset_funcs = {
    ...
    'laion_beyond': get_laion_beyond_datasets
}

get_dataset_funcs_with_gcdval = {
    'cifar10': get_cifar_10_datasets_with_gcdval,
    ...
    'laion_beyond': get_laion_beyond_datasets_with_gcdval
}
...
def get_datasets_with_gcdval(dataset_name, train_transform, test_transform, args):
    ...
    if dataset_name == 'laion_beyond':
        datasets = get_dataset_f(train_transform=train_transform, test_transform=test_transform,
                                train_classes=args.train_classes,
                                prop_train_labels=args.prop_train_labels,
                                split_train_val=True, val_split=0.1, subfield=args.subfield)
    else:
        datasets = get_dataset_f(train_transform=train_transform, test_transform=test_transform,
                            train_classes=args.train_classes,
                            prop_train_labels=args.prop_train_labels,
...
def get_class_splits(args):
    ...
    elif args.dataset_name == 'laion_beyond':
        args.image_size = 224
        oov_dir, iv_dir = find_dataset_info(laion_beyond_root, args.subfield)
        iv_mapping = load_merged_mapping(iv_dir)#每个类的所有图片字典
        oov_mapping = load_merged_mapping(oov_dir)
        old_class_count = len(iv_mapping)
        new_class_count = len(oov_mapping)
        args.train_classes = range(old_class_count)
        args.unlabeled_classes = range(old_class_count, old_class_count + new_class_count)
