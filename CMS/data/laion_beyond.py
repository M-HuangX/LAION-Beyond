    ...
    for mapping, is_oov in [(iv_mapping, False), (oov_mapping, True)]:
        for class_name, images in mapping.items():
            for img_name in images:
                ...
                dataset.targets.append(label)#类别标签
                dataset.uq_idxs.append(len(dataset.uq_idxs))#唯一标签
...
def get_laion_beyond_datasets(train_transform, test_transform, train_classes=(0, 1, 8, 9),
    ...
    if os.path.exists(split_file):
        ...
        full_dataset = create_dataset(root, oov_dir, iv_dir)#包含了IV和OOV的所有图片和类别标签，以及每个样本图片都有唯一的标签
        ...
        # 第一级分割（5/6 vs 1/6）
        总样本 = 800张
        test_dataset = 667张（每个类别约83张）
        remaining_dataset = 133张（每个类别约17张）
        
        # 第二级分割（旧类别vs新类别）
        remaining_dataset中：
        - old_indices: 85张（5个旧类别，每类约17张）
        - new_indices: 48张（3个新类别，每类约16张）
        
        # 第三级分割（标记vs未标记）
        old_dataset（85张）分割：
        - labelled_indices: 68张（80%的旧类别数据）
        - unlabelled_indices_old: 17张（20%的旧类别数据）
        
        最终未标记数据：
        unlabelled_indices = 17张（旧类别未标记）+ 48张（新类别）= 65张
        '''
        # Create new split
        oov_dir, iv_dir = find_dataset_info(root, subfield)
        full_dataset = create_dataset(root, oov_dir, iv_dir)
        
        # Split into test (5/6) and remaining (1/6) 每个类的5/6个样本划分到test，剩余给
