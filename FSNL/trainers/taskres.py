...
class TaskResLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model, base_text_features):
        super().__init__()
        self.device = clip_model.visual.conv1.weight.dtype
...
def _get_base_text_features(cfg, classnames, clip_model, text_encoder):
    ...
    with torch.no_grad():
        ...
        for text in classnames:
            tokens = open_clip.tokenizer.tokenize([template.format(text) for template in TEMPLATES])  # tokenized prompts are indices
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.dtype = clip_model.visual.conv1.weight.dtype   # float16
...
class TaskRes(TrainerX):
    ...
    def build_model(self):
        ...
        default_log_template["PREC"] = self.cfg.TRAINER.TaskRes.PREC
