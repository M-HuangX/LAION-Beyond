...
def load_clip_to_cpu(cfg):
    ...
    model, _, preprocess = open_clip.create_model_and_transforms(backbone_name, precision=cfg.TRAINER.COCOOP.PREC, pretrained='laion400m_e32')
    model.eval()
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if ctx_init:
            ...
            # OpenCLIP adaptation
            prompt = open_clip.tokenizer.tokenize(ctx_init)
            with torch.no_grad():
        ...
        # OpenCLIP adaptation
        tokenized_prompts = torch.cat([open_clip.tokenizer.tokenize(p) for p in prompts])  # (n_cls, n_tkn)
        with torch.no_grad():
...
class CoCoOp_openclip(TrainerX):
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.COCOOP.PREC == "fp32" or cfg.TRAINER.COCOOP.PREC == "amp":
            ...
            clip_model.float()
        print("Building custom Open_CLIP")
        ...
        default_log_template["PREC"] = self.cfg.TRAINER.COCOOP.PREC
