...
from .coop_openclip import load_clip_to_cpu
...
CUSTOM_TEMPLATES = {
    ...
    # "Pokemon": "a photo of a {} pokemon.",
    # "Animals103": "a photo of a {}, a type of animal.",
    # "Insects_Spiders108": "a photo of a {}, a type of insect or spider.",
    # "Plants_Fungi116": "a photo of a {}, a type of plant or fungus."


@TRAINER_REGISTRY.register()
class ZeroshotCLIP_openclip(TrainerX):
    def build_model(self):
        ...
        # Model info
        if self.cfg.DATASET.USE_SUPERCLASS_F_ZS_TEST:
            default_log_template["Method"] = str(self.cfg.TRAINER.NAME) + "_USE_SUPERCLASS"
        else:
            default_log_template["Method"] = str(self.cfg.TRAINER.NAME)
        default_log_template["PREC"] = self.cfg.TRAINER.COOP.PREC
        default_log_template["MAX_EPOCH"] = 0
        # Experiment settings
        default_log_template["SHOTS"] = 0
        default_log_template["LR"] = 0
        default_log_template["BATCH_SIZE"] = 0
...
@TRAINER_REGISTRY.register()
class ZeroshotCLIP2_openclip(ZeroshotCLIP_openclip):
    """Prompt ensembling."""
    ...
    def build_model(self):
        ...
        for i, temp in enumerate(self.templates):
            prompts = [temp.format(c.replace("_", " ")) for c in classnames]
            prompts = torch.cat([open_clip.tokenizer.tokenize(p) for p in prompts]).to(self.device)
            text_features = clip_model.encode_text(prompts)
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)
