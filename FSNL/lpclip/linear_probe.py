...
val_file_path = os.path.join(dataset_path, "val.npz")
test_file_path = os.path.join(dataset_path, "test.npz")

while not (os.path.exists(train_file_path) and os.path.exists(val_file_path) and os.path.exists(test_file_path)):
    print("Waiting for all npz files to appear...")
...
os.makedirs(os.path.join(dataset_path, "report"), exist_ok=True)

val_shot_list = {1: 1, 2: 2, 4: 4, 8: 4, 16: 4}

def log_results_to_csv(results, central_csv_file='central_experiment_results2.csv', single_data_folder='single_experiment_data2'):
        
    csv_log_root = "/data/chenziliang/output/csv_log_center/"
    central_csv_file = os.path.join(csv_log_root, central_csv_file)
    single_data_folder = os.path.join(csv_log_root, single_data_folder)

    # Ensure per-run output directory exists
    if not os.path.exists(single_data_folder):
        os.makedirs(single_data_folder)

    # Generate filename for this run based on timestamp
    timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S-%f")  # %f provides millisecond precision
    single_file = os.path.join(single_data_folder, f"experiment_{timestamp}.csv")

    # Write results to per-run file
    with open(single_file, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=results.keys())
        writer.writeheader()
        writer.writerow(results)

    # Append results to central aggregated file
    file_exists = os.path.isfile(central_csv_file)
    with open(central_csv_file, mode='a', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=results.keys())
        if not file_exists:
            writer.writeheader()
        writer.writerow(results)
...
for num_shot in [1, 2, 4, 8, 16]:
    ...
    default_log_template["Method"] = "Linear probe"
    default_log_template["PREC"] = "fp32"
    default_log_template["MAX_EPOCH"] = 8
    # Experiment settings
    default_log_template["SHOTS"] = num_shot
    default_log_template["LR"] = 0
    default_log_template["BATCH_SIZE"] = 128
    default_log_template["Datasets"] = dataset
    default_log_template["SUB_CLASSES"] = "OOP"
    default_log_template["Epoch"] = 8
    default_log_template["accuracy"] = acc_mean
    default_log_template["error_rate"] = acc_std
    default_log_template["macro_f1"] = acc_mean
    default_log_template["SEED"] = 4 # Filter out entries with value 2
    default_log_template["NOTE"] = ""
    log_results_to_csv(default_log_template)
