...
from datasets.Pokemon89_39 import Pokemon89_39
from datasets.Animals92_42 import Animals92_42
from datasets.Architecture50_23 import Architecture50_23
from datasets.Attire54_28 import Attire54_28
from datasets.FolkArt59_27 import FolkArt59_27
from datasets.Food53_27 import Food53_27
from datasets.Insects_Spiders106_52 import Insects_Spiders106_52
from datasets.Landmark59_30 import Landmark59_30
from datasets.Plants_Fugi113_56 import Plants_Fugi113_56
...
def main(args):
    ...
    setup_logger(os.path.join(cfg.OUTPUT_DIR, cfg.DATASET.NAME))
    ...
    if args.split == "train":
        ...
        is_train=False
    tfm_train = build_transform(cfg, is_train=is_train)
    data_loader = torch.utils.data.DataLoader(
        DatasetWrapper(cfg, dataset_input, transform=tfm_train, is_train=is_train),
    ...
    clip_model, _, preprocess = open_clip.create_model_and_transforms(
        "ViT-B-16", precision='fp32', pretrained='laion400m_e32')
    clip_model.eval()
    clip_model.to("cuda")
    ...
    for train_step in range(1, len(train_dataiter) + 1):
        ...
        data = batch["img"].cuda()
        feature = clip_model.visual(data.type(dtype))
        feature = feature.cpu()
        for idx in range(len(data)):
            feature_list.append(feature[idx].tolist())
        label_list.extend(batch["label"].tolist())
    save_dir = os.path.join(cfg.OUTPUT_DIR, cfg.DATASET.NAME, cfg.DATASET.SUB_CLASSES)
